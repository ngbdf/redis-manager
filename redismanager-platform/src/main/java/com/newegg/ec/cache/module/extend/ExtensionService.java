...
public class ExtensionService implements ApplicationListener<ContextRefreshedEvent>,IExtensionService {
        ...
        if (memAllocator.startsWith("jemalloc") && (Integer.valueOf(redisVersion.substring(0, 1)) >= 4)) {
            ...
            try {
                if (StringUtils.isNotBlank(password)) {
                    result = redisClient.redisCommandOpt(password, RedisExtendClient.MEMORYPURGE);
                } else {
                    result = redisClient.redisCommandOpt(RedisExtendClient.MEMORYPURGE);
                }
    ...
    private class MemoryDiagnosis implements Callable<String>{
        ...
        public String call() throws Exception {
            ...
            try {
                if (StringUtils.isNotBlank(password)) {
                    result = redisClient.redisCommandOpt(password, RedisExtendClient.MEMORYDOCTOR);
                } else {
                    result = redisClient.redisCommandOpt(RedisExtendClient.MEMORYDOCTOR);
                }
            } catch (Exception e) {
