...
public class RedisInfoChecker {
     ...
     */
    private class SlowLogCheckTask implements Runnable {
        ...
        public void run() {
            ...
            int port = Integer.parseInt(host.split(":")[1]);
            try {
                List<Object> list = RedisUtils.getSlowLog(new RedisConnectParam(ip, port, cluster.getRedisPassword()), 2000);
                for (Object  slowlog : list) {
                    JSONArray array = JSONArray.fromObject(slowlog);
                    //todo
                    /*if (array.getLong("timestamp")> time) {
                        num++;
                    }*/
                }
                if (num >= 100) {
