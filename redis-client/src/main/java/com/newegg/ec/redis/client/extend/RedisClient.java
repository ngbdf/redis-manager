package com.newegg.ec.redis.client.extend;
 ...
 *
 * jedis客户端没有提供某些命令的API(如4.0以后Memory下的子命令等)，可以依据RESP协议简单实现这些的API
 * 开箱即用，用完关闭客户都即可
 */
public class RedisClient {
    ...
    public RedisClient(String host, int port){
        try {
            ...
            throw new RedisClientException("Create RedisClient Error",e);
        }
    }
    ...
    public String redisCommandOpt(String pssword,String command) throws IOException, InterruptedException {
        ...
        //在有密码的集群执行command时，需要延时一定的时间确保redis的response结果完全写入inputStream中
        Thread.sleep(20);
        inputStream.read(bytes);
        return new String(bytes);
    ...
    public void closeClient() {
        ...
        try {
            ...
            throw new RedisClientException("Close RedisClient Error",e);
        }
